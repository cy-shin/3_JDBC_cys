...
public class JDBCExample5 {
	public static void main(String[] args) {
		...
		// TIP : SQL문을 먼저 생각해보고, 그 후에 자바를 작성하면 도움이 된다
		...
		try {
			...
			while(rs.next()) {
				...
				emp.setEmpName( rs.getString("이름") ); // 조회 시 컬럼명이 "이름"이므로 별도의 select문 필요 없음
				emp.setHireDate( rs.getString("입사일") );
				// 현재 행의 "입사일" 컬럼의 문자열 값을 얻어와 emp가 참조하는 객체의 hireDate 필드에 세팅
				
				emp.setGender(rs.getString("성별").charAt(0));
//				emp.setGender(rs.getChar);
				// - char 자료형 매개변수가 필요함
				// *중요*
				// Java의 char 문자 1개 의미
